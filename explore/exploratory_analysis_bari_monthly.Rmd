...
```{r, fig.cap="Percent of patients discharged on post-op day 1 by type of surgery performed"}
data_patients %>%
    group_by(surg_month) %>%
    add_count(surgery) %>%
...
data_revisit %>%
    ...
    geom_line(aes(color = visit.type)) +
    geom_text_repel(
        aes(label = label), 
        size = 3,
        color = "grey35",
