...
include %>%
    ...
    datatable(caption = "Location where pain medications are given",
              ...
              options = list(dom = "t")) %>%
    formatPercentage(c("baseline_pct_doses", "current_pct_doses"), 1)
```
...
hdr_meds <- htmltools::withTags(table(
    ...
    thead(
        tr(
            ...
            th(colspan = 4, 'Number of Doses'),
            th(colspan = 4, 'Median Total Dose')
        ),
        tr(
            th(colspan = 2, 'Baseline'),
            th(colspan = 2, 'Current'),
...
include %>%
    ...
    unite(group_test, group, test) %>%
    spread(group_test, value) %>%
    select(med, route, baseline_n, baseline_med_pct, current_n, current_med_pct, baseline_num_doses_median, baseline_num_doses_IQR, current_num_doses_median, current_num_doses_IQR, baseline_med.dose_median, baseline_med.dose_IQR, current_med.dose_median, current_med.dose_IQR) %>%
    datatable(caption = "Pain Medications (All Locations)",
              rownames = FALSE,
              container = hdr_meds,
              options = list(dom = "t")) %>%
    formatPercentage(c("baseline_med_pct", "current_med_pct"), 1)
```
