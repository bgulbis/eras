...
dir_tidy <- paste0("../data/tidy/gynonc/", begin_abbrev, "_", end_abbrev)
...
* Patients admitted between July 1, 2017 and June 30, 2018
* ICD-10 Procedure Codes: 0UT90ZZ, 0UTC0ZZ, 0UT70ZZ, 0UT20ZZ, 0DBS0ZX, 0DBS0ZZ, 0UT00ZZ, 0UT10ZZ, 0UT60ZZ, 0UT50ZZ, 0UB20ZZ, 0UDB8ZZ, 0UB98ZZ, 0UBM0ZZ, 0UT54ZZ, 0UB04ZZ, 0DBW0ZX

```{r, fig.cap="Number of patients undergoing surgery each month", fig.height=3.5}
data_patients %>%
    ungroup() %>%
    mutate_at(
        "surgery",
...
```{r, fig.cap="Length of stay trend by surgeon.", fig.height=3.5}
df <- data_patients %>%
    select(
        millennium.id, 
        surg_month, 
...
df %>%
    ...
    scale_y_continuous("Length of stay (days)", breaks = 0:10) +
    # coord_cartesian(ylim = c(0, 8)) +
    theme_bg() 
...
```{r, fig.cap="Percent of patients discharged on post-op day 1 by type of surgery performed", fig.height=8, eval=FALSE}
data_patients %>%
    group_by(surg_month) %>%
    add_count(surgery) %>%
...
```{r, fig.cap="Distribution of total IV morphine equivalents received per patient on the floor during the first 24 hours post-op.", fig.height=3.5}
df_opiods %>%
    filter(timing == "Floor") %>%
