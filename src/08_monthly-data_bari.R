...
dir_raw <- paste0("data/raw/report/", begin_abbrev, "_", end_abbrev)
...
bari_pts <- read_data(dir_raw, "patients_eras-bari", FALSE) %>%
    as.patients()

mbo_id <- concat_encounters(bari_pts$millennium.id)
...
bari_visit <- read_data(dir_raw, "^visit", FALSE) %>%
    as.visits() %>%
    filter(admit.type == "Preadmit Not OB") %>%
    left_join(
        bari_id[c("millennium.id", "pie.id")],
        by = "millennium.id"
    )
...
bari_locations <- read_data(dir_raw, "locations", FALSE) %>%
    as.locations() %>%
    tidy_data() %>%
    semi_join(bari_visit, by = "millennium.id")
...
bari_pts <- semi_join(bari_pts, data_patients, by = "millennium.id")
...
bari_encounters <- read_data(dir_raw, "encounters") %>%
    ...
    left_join(
        bari_pts[c("millennium.id", "discharge.datetime")],
        by = "millennium.id"
    ) %>%
...
bari_revisit_mbo <- concat_encounters(bari_encounters$millennium.id)
...
data_revisit <- read_data(dir_raw, "revisit", FALSE) %>%
    ...
    left_join(
        bari_id[c("millennium.id", "pie.id", "person.id")],
        by = "person.id"
    ) %>%
